...
const TOKEN_API_URL = "https://dev.developers-test.currencybird.cl/token?email=fjvega1@uc.cl";
const TRANSFER_API_URL = "https://dev.developers-test.currencybird.cl/payment?email=fjvega1@uc.cl&transferCode=fjvega1@uc.cl";
...
const createTransfer = async (transferCode: string, amount: number, authToken: string): Promise<AxiosResponse<TransferApiResponse>> => {
  try {
    ...
    const externalApiResponse = await axios.post<TransferApiResponse>(TRANSFER_API_URL, transferRequestBody, { headers });
...
const getToken = async () => {
  try {
    ...
    const tokenApiResponse = await axios.get(TOKEN_API_URL);
    ...
    if (tokenApiResponse.data) {
      ...
      console.error("Unexpected response status when retrieving the token:", tokenApiResponse.status);
    }
  } catch (error) {
    // Handle errors and log them
...
router.post('/create-money-transfer', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const existingTransfer = await Transfer.findOne({ transferCode });
    ...
    const externalApiResponse = await createTransfer(transferCode, amount, authToken);
    ...
    if (externalApiResponse.data) {
      ...
      try {
        ...
        res.status(500).json({ error: 'Error saving the transfer in the local database.' });
      }
    } else {
      // Handle errors when the external API response is not valid
      res.status(400).json({ error: 'Error creating the transfer in the external API.' });
    }
  } catch (error) {
    // Handle errors related to checking the local database
    res.status(500).json({ error: 'Error verifying the existence of the transfer in the local database.' });
  }
});
